...
if (isset($_POST['submit'])) {
    try {
        ...
        $stmt_update = $pdo->prepare($sql_update);
          //bindValue()メソッドを使い、受け取った商品のid（$_GET['id']）をプレースホルダ（:id）にバインドする（割り当てる）実際の値
      ...
      header("Location: read.php?message={$message}");
        ...
        // セレクトボックスの選択肢として設定するため、コードの配列を取得する
        if (isset($_GET['id'])) {
          try {
            $pdo = new PDO($dsn, $user, $password);
            ...
            //prepare()メソッド。【結果を返すクエリ。動的に変わる値】の実行
            ...
            $stmt_select_genre_codes = $pdo->query($sql_select_genre_codes);
             ...
             // SQL文の実行結果を配列で取得する
            // 補足：(PDO::FETCH_COLUMN)は1つのカラムの値を1次元配列（多次元ではない普通の配列）で取得する設定.
    ...
    <article class="registration">
      ...
      <form action="update.php?id=<?= $_GET['id'] ?>"method="post" class="registration-form">
        <div>
          ...
          <input type="number" id="price" name="price" value="<?= $book['book_code'] ?>"min="0" max="100000000" required>

          <label for="stock_quantity">在庫数</label>
          <input type="number" id="stock_quantity" name="stock_quantity" value="<?= $book['book_code'] ?>"min="0" max="100000000" required>

          <label for="genre_code">ジャンルコード</label>
          <select id="genre_code" name="vendor_code" required>
            ...
            if ($genre_code === $product['gender_code']) {
